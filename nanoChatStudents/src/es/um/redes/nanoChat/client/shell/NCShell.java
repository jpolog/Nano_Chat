	...
	private String[] readChatCommandFromStdIn(NCConnector ncclient) {
		...
		while (true) {
			...
			case NCCommands.COM_SEND:
				...
				while (st.hasMoreTokens())
					message1.append(st.nextToken()+" ");
				String msg1 = message1.toString();
				if (!msg1.equals(""))	//Si no hay parámetros no se añaden
					vargs.add(msg1);
				break;
			case NCCommands.COM_SEND_PRIVATE:
				...
				String msg2 = message2.toString();
				if (!msg2.equals(""))	//Si no hay parámetros no se añaden
					vargs.add(msg2);
				break;
