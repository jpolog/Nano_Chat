...
public class NCServerThread extends Thread {
			...
			while (true) {
				...
				case NCMessage.OP_ENTER_ROOM: {
					...
					if (roomManager == null) {
						...
						sendMessage(NCMessage.OP_ENTER_ROOM_NOTIFICATION, user, null);
	...
	private void processRoomMessages() throws IOException  {
		...
		while (!exit) {
			...
			NCMessage message = NCMessage.readMessageFromSocket(dis);
			//Se mantiene actualizado el nombre de la sala por si algún usuario lo ha cambiado
			currentRoom = this.roomManager.getDescription().roomName;
			...
			case NCMessage.OP_EXIT_ROOM: {
				...
				sendMessage(NCMessage.OP_EXIT_ROOM_NOTIFICATION, user, null);
			...
			case NCMessage.OP_TEXT_MESSAGE_OUT: {
				String text = ((NCTextMessage)message).getText();
				sendMessage(NCMessage.OP_TEXT_MESSAGE_IN, user, text);
				Date time = new Date();
				serverManager.setTime(currentRoom, time.getTime());
				
				break;
				
			}
			case NCMessage.OP_PRIVATE_TEXT_MESSAGE_OUT: {
				String nick = ((NCNickMessage)message).getNick();	//Receptor del mensaje
				String text = ((NCNickMessage)message).getText();
				sendPrivateMessage(nick, text);
				break;
	...
	private void sendPrivateMessage(String receiver,  String text) throws IOException  {
		serverManager.sendPrivateMessage(currentRoom, user, receiver, text);
	}
